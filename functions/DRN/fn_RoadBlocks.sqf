...
if (count _this > 1) then { _side = _this select 1; } else { _side = resistance; };
...
//    if (_side == A3E_VAR_Side_Ind) then {
        _possibleVehicles = arr_RoadBlock_MannedVehicleTypes;
//    };
    ...
    _gun = arr_StaticWeaponTypes select floor random count arr_StaticWeaponTypes;
   // private _static = [_gun, (_pos vectoradd [10,0,0]), _dir, _centerPos, _rotateDir] call _fnc_CreateObject;
	...
	_units pushback ([_static,_side] call fnc_AddStaticGunner); 
	// A3E_fnc_Roadblock ["_center","_rotation","_static","_vehicle"];
	_units append ([_pos,(_dir + _angle),_static,_vehicle] call fnc_Roadblock);
...
//    };
	...
	[_group, _marker] spawn fnc_Patrol;
...
while {true} do {
    ...
    _roadBlocksDeleted = 0;
    {
		...
		private _players = [] call fnc_getPlayers;
