...
if (count _this > 4) then {_minSpawnDistance = _this select 4;} else {_minSpawnDistance = 500;};
if (count _this > 5) then {_maxSpawnDistance = _this select 5;} else {_maxSpawnDistance = 1000;};
...
while {true} do {
    ...
    while {count _activeGroups < _groupsCount} do {
        ...
        _spawnPos = [units _referenceGroup, _minDistance, _maxSpawnDistance] call fnc_RandomSpawnPos;
...
//			[_x, a3e_var_Escape_enemyMinSkill] call EGG_EVO_skill;
		...
		_script = [_group, nil] spawn fnc_Patrol;
