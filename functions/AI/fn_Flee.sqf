...
[_group,"FLEE"] call a3e_fnc_SetTaskState;
	...
	_homeMarker = _group getvariable "homeMarker";
	if(!isNil("_homeMarker")) then {
		_oncomplete = format["[group this,""%1""] spawn fnc_Patrol;",_homeMarker];
	} else {
		_oncomplete = "[group this,nil] spawn fnc_Patrol;";
	};


_waypoint = [_group,_targetposition,"MOVE","LINE","FULL","AWARE",_oncomplete] call fnc_move;
