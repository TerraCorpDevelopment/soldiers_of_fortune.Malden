...
{
	case "Все": {(VVS_pre_Car + VVS_pre_Air + VVS_pre_Ship + VVS_pre_Submarine + VVS_pre_Armored + VVS_pre_Support + VVS_pre_Autonomous)};
	case "Машины": {VVS_pre_Car};
	case "Воздух": {VVS_pre_Air};
	case "Лодки": {VVS_pre_Ship};
	case "Субмарины": {VVS_pre_Submarine};
	case "Броня": {VVS_pre_Armored};
	case "БПА": {VVS_pre_Autonomous};
	case "Поддержка": {VVS_pre_Support};
	default {[]};
};
